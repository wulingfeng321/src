...
void ImageGrabber::SyncWithImu()
{
    ...
    while(1)
    {
        ...
        if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
        {
            ...
            // 创建一个与原图大小相同的黑色图像
            cv::Mat black_image = cv::Mat::zeros(imLeft.size(), imLeft.type());
            std::cout << "imu size:"<< vImuMeas.size() << std::endl;

            // 使用addWeighted函数进行混合，alpha值越小图像越暗
            double alpha = 0.4;  // 控制原图的权重
            double beta = 1.0 - alpha;  // 黑色图像的权重

            // cv::addWeighted(imLeft, alpha, black_image, beta, 0.0, imLeft);
            // cv::addWeighted(imRight, alpha, black_image, beta, 0.0, imRight);
            cv::equalizeHist(imLeft, imLeft);
